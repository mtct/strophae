...
export function registerIpc(store: Store, dir: string,
  ...
  // `images` are data: URLs generated by the model during this stream;
  // they are persisted as image attachments of the assistant message.
  ipcMain.handle('msg:finalize',
    (_e, messageId: number, text: string, images: string[] = []) => {
      const atts: Attachment[] = [];
      for (const url of images) {
        try {
          atts.push(importDataUrl(dir, store.claimId(), url));
        } catch { /* skip malformed/oversized image */ }
      }
