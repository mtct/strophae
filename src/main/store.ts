...
import { CATALOGS, translate, type Lang } from '../shared/i18n';
import {
...
// Every persona the app ships, and the languages they can be materialised
// in — used to keep the still-pristine ones in step with the UI language.
const SEED_PERSONAS: SeedPersona[] = [...SIX_HATS, ...MEDIA_PERSONAS];
const SEED_LANGS = Object.keys(CATALOGS) as Lang[];
import type {
  Agent,
  Attachment,
  Conversation,
...
export class Store {
  ...
  setLanguage(language: Language): void {
    ...
    this.retranslateSeedPersonas(this.lang());
    this.save();
  }

  /** Bring the untouched seed personas into `lang` when the UI language
      changes, so the default library reads in the same language as the rest
      of the app. A persona is re-translated only when it is still *pristine*
      — its name and prompt both equal the seed's canonical text in some
      shipped language — so anything the user renamed, re-prompted or created
      from scratch is left exactly as it is. Deleted seed personas are simply
      absent and stay gone. */
  private retranslateSeedPersonas(lang: Lang): void {
    for (const p of this.data.personas) {
      const seed = SEED_PERSONAS.find((s) => s.personaType === p.personaType);
      if (!seed) continue; // a custom persona, never one of ours
      const pristine = SEED_LANGS.some((l) =>
        p.name === translate(l, seed.nameKey)
        && p.systemPrompt === translate(l, seed.promptKey));
      if (!pristine) continue; // user-edited — leave their wording untouched
      p.name = translate(lang, seed.nameKey);
      p.systemPrompt = translate(lang, seed.promptKey);
    }
  }
